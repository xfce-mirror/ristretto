...
rstto_thumbnailer_init (RsttoThumbnailer *thumbnailer)
{
    ...
    thumbnailer->priv->image_list = NULL;
...
rstto_thumbnailer_finalize (GObject *object)
{
    ...
    g_clear_object (&thumbnailer->priv->image_list);
        ...
        RsttoFile *file)
{
    ...
    thumbnailer->priv->queue = g_slist_remove_all (thumbnailer->priv->queue, file);
    g_object_unref (file);
...
rstto_thumbnailer_set_image_list (RsttoThumbnailer *thumbnailer,
                                  RsttoImageList   *image_list)
{
    if (thumbnailer->priv->image_list != NULL)
        g_object_unref (thumbnailer->priv->image_list);

    thumbnailer->priv->image_list = g_object_ref (image_list);
}
