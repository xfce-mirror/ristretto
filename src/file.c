...
rstto_file_needs_materialization (RsttoFile *r_file)
{
    ...
    if (g_strcmp0 (uri, "file:///dev/stdin") == 0)
        needs = TRUE;
                        ...
                        GError **error)
{
    ...
    gchar *path = g_file_get_path (r_file->priv->file);
    if (g_strcmp0 (path, "/dev/stdin") == 0)
    {
        GFileIOStream *io;
        GFile *tmp_file = g_file_new_tmp ("ristretto-XXXXXX", &io, &tmp_error);
        if (tmp_error == NULL)
        {
            GFileInputStream *in = g_file_read (r_file->priv->file, NULL, &tmp_error);
            if (tmp_error == NULL)
            {
